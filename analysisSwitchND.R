...
df3centroidsEngineer <- aggregate(df3EngineerRemoveNarrator,by=list(df3EngineerRemoveNarrator$groups),FUN="mean")
pdf(paste(figPath,"mdsdim1dim2ellipseengineer.pdf",sep=""),width = 10, height = 10)
#plot(dim1nm,dim2nm,type="n")
#text(dim1nm, dim2nm, labels = row.names(data), cex=.7)
print(
ggplot(df3EngineerRemoveNarrator,aes(x = MDS1,y = MDS2,label=rownames(df3EngineerRemoveNarrator),color=factor(df3EngineerRemoveNarrator$groups))) + stat_ellipse(geom = "polygon", alpha = 1/2, aes(fill = factor(groupsEngineer))) + geom_point(size=1) + theme_bw() + xlim(-0.2,0.2) + ylim(-0.2,0.2) + labs(x = "Dimension I",y = "Dimension II") + scale_color_discrete(name = "Group",labels = groupLabs)  + ggtitle(material) + geom_text(data=df3centroidsEngineer,aes(x = MDS1,y = MDS2,label=c("1","Non-diegetic music","Transients coinciding \n with onscreen action","Individually\n identifiable\n background"),color=factor(df3centroidsEngineer$groups)),fontface="bold") + theme(legend.position = "none")
)# 
#)
#plot(nmmds,type="t") + xlim(-2,2) + ylim(-2,2)
dev.off()

pdf(paste(figPath,"mdsdim1dim2textengineernarratorremoved.pdf",sep=""),width = 10, height = 10)
#plot(dim1nm,dim2nm,type="n")
#text(dim1nm, dim2nm, labels = row.names(data), cex=.7)
print(
ggplot(df3EngineerRemoveNarrator,aes(x = MDS1,y = MDS2,label=rownames(df3EngineerRemoveNarrator),color=factor(df3EngineerRemoveNarrator$groups))) + geom_text(size = textSize) + theme_bw() + xlim(-0.2,0.2) + ylim(-0.2,0.2) + labs(x = "Dimension I",y = "Dimension II") + scale_color_discrete(name = "Group",labels = groupLabs) + theme(legend.position = c(.9,.9)) + ggtitle(material)
)
#plot(nmmds,type="t")
dev.off()
